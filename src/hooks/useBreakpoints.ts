import { PIXEL_BREAKPOINT_VALUES } from "src/styles/tokens";
...
export const useBreakpoints = (): Breakpoints => {
  ...
  const [breakpoints, setBreakpoints] = useState<Breakpoints>(
    getBreakpointValues(width)
  );
  const updateBreakpoints = useCallback(
    () => setBreakpoints(getBreakpointValues(width)),
    [width]
  );
  useEffect(() => {
    updateBreakpoints();
...
const { MOBILE_UP_TO, TABLET_UP_TO, DESKTOP_S_UP_TO, DESKTOP_M_UP_TO } =
  PIXEL_BREAKPOINT_VALUES;
